...
/**
 * Página pública de cardápio (nokta.live/cardapio/{slug}) — layout portado
 * fielmente de uma referência visual de mercado (hero escuro com anel,
 * avatar circular, chips de categoria sticky, 3 modos de visualização,
 * favoritos reais). Mobile-first (o público real é 100% celular, em
 * evento/bar ninguém abre no notebook), mas ajustado com breakpoints
 * md/lg/xl para tablet/desktop em vez de ficar preso a um cartão de 440px
 * centralizado.
 */
export function PublicMenuView({ initialData, orgSlug }: { initialData: PublicMenuResponse; orgSlug: string }) {
  ...
  const [activeCategoryId, setActiveCategoryId] = useState<number | "highlights">(
    initialData.menu.highlights.length > 0 ? "highlights" : (initialData.menu.categories[0]?.id ?? "highlights"),
  );
  ...
  // A página inteira rola dentro do <main fixed inset-0 overflow-y-auto> do
  // Root Layout (ver page.tsx), nunca no `window`/`body` — por isso o
  // listener precisa ir no próprio elemento scrollável, achado subindo a
  // árvore a partir do profile (chips têm `sticky top-0`, cujo
  // getBoundingClientRect().top gruda em 0 quando colado e nunca reflete
  // quanto realmente rolou).
  useEffect(() => {
    const scrollParent = profileRef.current?.closest("main");
          ...
          {itemsToShow.length === 0 ? (
            ...
            <div className="grid grid-cols-2 gap-4 lg:grid-cols-3">
              {itemsToShow.map((item) => (
                <ItemGridCard key={item.id} item={item} onToggleFavorite={() => toggleFavorite(item)} />
              ))}
            </div>
          ) : view === "large" ? (
            <div className="grid grid-cols-1 gap-6 md:grid-cols-2 lg:grid-cols-3">
              {itemsToShow.map((item) => (
                <ItemLargeCard key={item.id} item={item} onToggleFavorite={() => toggleFavorite(item)} />
              ))}
            </div>
          ) : (
            <div className="divide-y divide-[#ececee]">
              {itemsToShow.map((item) => (
                <ItemListRow key={item.id} item={item} onToggleFavorite={() => toggleFavorite(item)} />
              ))}
            </div>
          )}
        </div>
